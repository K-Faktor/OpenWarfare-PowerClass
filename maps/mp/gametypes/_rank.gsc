...
giveRankXP( type, value )
{
	...
	if ( isDefined( self.enableText ) && self.enableText && ( !level.hardcoreMode || (level.scr_hud_show_xp_points == 2)))
	{
		if ( type == "teamkill" )
...
updateRank()
{
	...
	{	
		...
		if ( isDefined( unlockedWeapon ) && unlockedWeapon != "" )
		{
			...
			self thread openwarfare\_powerclass::unlockweapon( unlockedWeapon );
		}
		...
		if ( isDefined( unlockedPerk ) && unlockedPerk != "" )
		{
			...
			self thread openwarfare\_powerclass::unlockPerk( unlockedPerk );
		}
		...
		if ( isDefined( unlockedAttachment ) && unlockedAttachment != "" )
		{
			...
			self thread openwarfare\_powerclass::unlockAttachment( unlockedAttachment );
		}
...
updateRankScoreHUD( amount )
{
	...
	if( isDefined( self.hud_rankscroreupdate ) && level.scr_hud_show_xp_points != 0 )
	{			
