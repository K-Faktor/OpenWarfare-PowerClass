...
init()
{
	...
	// Tally - generate some arrays for progressive ranking
	level.baseWeaponName = [];
	...
	for( wi = 0; wi <= max_weapon_num; wi++ )
	{
		...
		// Tally - we don't want the weapon + suffix - just the base weapon name.
		level.baseWeaponName[level.baseWeaponName.size] = weapon_name;
		...
		if( attachment_tokens.size == 0 )
		{
			...
			// Tally - add the attachment to an array
			level.weaponAttachments[level.weaponAttachments.size] = attachment;
		...
		else
		{
			for( k = 0; k < attachment_tokens.size; k++ )
			{
				...
				// Tally - add the attachment to an array
				level.weaponAttachments[level.weaponAttachments.size] = attachment_tokens[k];
	...
	{
		if( bypassPrecache( level.weaponList[index] ) )
			continue;
...
bypassPrecache( weapon )
{
	if( weapon == "defaultweapon_mp" || weapon == "defaultpistol_mp" )
		return true;
...
{
	return canDropWeapon( weapon );
}

canDropWeapon( weapon )
{	
...
{
	if( !canDropWeapon( self getCurrentWeapon() ) ) 
		return;	
