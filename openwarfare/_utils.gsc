...
byPassClassSelection()
{
	if( level.oldschool || level.gametype == "gg" 
	|| level.gametype == "ss" || level.gametype == "oitc" )
		return true;
...
GetPerkWeapon( perkname )
{	
	return( tableLookup( "mp/statsTable.csv", 0, level.perkReferenceToIndex[ perkname ], 4 ) );
}

convertPerktoStat( perkname )
{
	return( int( tableLookup( "mp/statsTable.csv", 4, perkName, 0 ) ) );
}

convertStattoPerk( perkStat )
{
	return( tableLookup( "mp/statsTable.csv", 0, perkStat, 4 ) );
}
...
convertStattoWeapon( weapStat )
{
	return( tableLookup( "mp/statsTable.csv", 0, weapStat, 4 ) );
}

getWeaponStat( weapon )
{
	return( int( tableLookup( "mp/statsTable.csv", 4, weapon, 0 ) ) );
}
...
getAttachmentName( attachmentStat )
{	
	return( tableLookup("mp/attachmentTable.csv", 9, attachmentStat, 4) );
}

getAttachmentStatfromName( attachmentName )
{
	return( int( tableLookup("mp/attachmentTable.csv", 4, attachmentName, 9) ) );
}
...
getCamoStat( camoName )
{
	return( int( tableLookup("mp/attachmentTable.csv", 4, camoName, 11) ) );
}

getCamoName( camoStat )
{
	return( tableLookup("mp/attachmentTable.csv", 11, camoStat, 4) );
}

WeaponHasAmmo( item )
{
	if( self getAmmoCount( item ) > 0 )
		return true;
